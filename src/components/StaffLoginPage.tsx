...
import { FaUserAlt, FaLock } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import staffData from "../data/staffData"; // adjust path as needed
...
const StaffLoginCard: React.FC<Props> = ({ onClose }) => {
  ...
  const handleLogin = () => {
    if (
      ...
      // Save admin login info if needed
      localStorage.setItem("loggedInAdmin", JSON.stringify({ username: ADMIN_USERNAME }));
      navigate("/admin-dashboard");
      return;
    }
